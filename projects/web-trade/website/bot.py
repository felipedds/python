...
class StrategyRSI(Bot):
    def __init__(self, ticker: str, start: datetime, end: datetime, time_frame: str):
        super().__init__(ticker, start, end, time_frame)
        self.strategy_rsi()
        
    def load_data(self): # Load dataset.
        dataset = Bot.load_data(self, self.ticker, self.time_frame)
...
class StrategyEMA(Bot):
    ...
    def strategy_ema(self): # Strategy EMA(Exponential Moving Average).
        ...
        if (ema9.iloc[:1].values > close.iloc[:1].values and ema21.iloc[:1].values > close.iloc[:1].values): # RSI 34 above 70.
            return f'Sell - Close: {close.iloc[:1].values} - EMA9 {ema9.iloc[:1].values} - EMA21 {ema21.iloc[:1].values} {self.ticker} {self.time_frame}.'
        if (ema9.iloc[:1].values < close.iloc[:1].values and ema21.iloc[:1].values < close.iloc[:1].values): # RSI 34 above 70.
            return f'Buy - Close: {close.iloc[:1].values} - EMA9 {ema9.iloc[:1].values} - EMA21 {ema21.iloc[:1].values} {self.ticker} {self.time_frame}.' 